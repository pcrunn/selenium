...
public class SimpleSeleniumService implements SeleniumService {
    ...
    private final Map<Player, SeleniumUI> players;

    public SimpleSeleniumService(JavaPlugin plugin) {
        instance = this;

        this.plugin = plugin;
        this.plugin.getServer().getPluginManager().registerEvents(new UIListener(this), plugin);
    ...
    public void openUI(Player player, SeleniumUI ui) {
        ...
        Map<UIItem, Integer> items = ui.getItems(player);
